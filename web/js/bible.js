// Bible metadata (KJV versification) + passage autocomplete for scripture
// fields. BOOKS entries are [name, versesPerChapter[]]. All data is local.
window.LJBible = (function () {
  const BOOKS = [
  ...
  // "1 jo" matches "1 John"; "song" matches "Song of Solomon".
  function normalize(s) { return s.toLowerCase().replace(/\s+/g, ' ').trim(); }
  function bookMatches(q) {
    const n = normalize(q);
    if (!n) return [];
    return BOOKS.filter(function (b) { return normalize(b[0]).startsWith(n); })
      .sort(function (a, b) { return a[0].length - b[0].length; });
  }
  function findBook(name) {
    const n = normalize(name);
    for (const b of BOOKS) if (normalize(b[0]) === n) return b;
    return null;
  }

  // Suggest completions for a partial reference:
  //   "jo"       -> books (John, Joel, Jonah, Joshua, Job…)
  //   "John "    -> chapters 1..21
  //   "John 1"   -> chapters 1, 10..19, 21
  //   "John 3:"  -> verses 1..36
  //   "John 3:1" -> verses 1, 10..19 … of 36
  ...
  function suggest(input) {
    ...
    // Chapter stage: book chosen, no colon yet.
    if (!colon) {
      const pre = chDigits || '';
      const out = [];
    ...
    const ch = parseInt(chDigits, 10);
    if (!ch || ch < 1 || ch > chapters.length) return [];
